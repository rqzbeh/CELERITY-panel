...
docker compose up -d
```
В этом `docker-compose.yml` панель теперь стартует на `127.0.0.1:3000` (только HTTP). До доступа к панели по HTTPS обязательно настройте TLS и проксирование через внешний Nginx/Traefik/Caddy на хосте.

**3. Настройте reverse proxy на хосте**, чтобы проксировать `https://ваш-домен` на `http://127.0.0.1:3000` (обязательно для установки через Docker Compose из исходников).
...
- **Основной порт (8443)** — порт, на котором слушает Hysteria
...
Можно запустить панель и ноду на одном VPS (панель TCP, нода UDP на 8443).
...
| `port`               | Number     | Основной порт (8443)                               |
